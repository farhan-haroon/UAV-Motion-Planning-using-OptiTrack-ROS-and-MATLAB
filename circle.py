...
class execute_circle:
    ...
    def publish_position_target(self):
        ...
        while not rospy.is_shutdown() and not(self.pose.pose.position.x < -0.1 and self.pose.pose.position.x > -0.7 and self.pose.pose.position.y < -0.1 and self.pose.pose.position.y > -0.7):
            ...
            position_target_msg.velocity.x = 0.2
